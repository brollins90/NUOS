...
void init_uart()
{
    ...
    /* Register the UART's interrupt handler. */
    register_interrupt_handler(57, (interrupt_handler_t)uart_irq_handler);    
}

int uart_receive()
{
    // disable uart interrupts for now
    unsigned int uart_imsc = uart->UART_IMSC;
    uart->UART_IMSC = 0;
    
    // wait until ready to receive
    while(uart->UART_FR & (1<<4)) { }
